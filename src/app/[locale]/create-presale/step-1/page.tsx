...
export default function CreatePresaleStep1() {
    ...
    const calculateAutoListing = () => {
        if (
            ...
            let total: number ;
            if (form.listingOption === ListingOption.AutoListing) {
                total =
                    hardCap * presaleRate +
                    (hardCap * 0.95 * listingRate * liquidityPercent) / 100;
            } else {
                total = hardCap * presaleRate
            }



            return `${total.toLocaleString()} ${form?.tokenInfo?.name}`;
        } else {
            return "?";
        }
    };
                ...
                <div className="grid grid-cols-2 gap-6">
                    ...
                    <div>
                        <Input
                            ...
                            label="PRESALE rate"
                            placeholder="0"
                            value={form?.presaleRate ? Number(form?.presaleRate).toLocaleString() : ''}
                            onBlur={() => {
                                if (form?.presaleRate) {
                                    handleChangeForm({
                                        presaleRate: Number(form?.presaleRate.toString().replace(/,/g, '')),
                                    });
                                } else {
                        ...
                        {form?.listingOption === "Auto Listing" && (
                            <>
                                <Input
                                    ...
                                    label="LISTING rate"
                                    placeholder="0"
                                    value={form?.listingRate ? Number(form?.listingRate).toLocaleString() : ''}
                                    onBlur={() => {
                                        if (form?.listingRate) {
                                            handleChangeForm({
                                                listingRate: Number(form?.listingRate.toString().replace(/,/g, '')),
                                            });
                                        } else {
                    ...
                    <div>
                        <Input
                            ...
                            label="Softcap"
                            placeholder="0"
                            value={form?.softCap ? Number(form?.softCap).toLocaleString() : ''}
                            onBlur={() => {
                                if (form?.softCap) {
                                    handleChangeForm({
                                        softCap: Number(form?.softCap.toString().replace(/,/g, '')),
                                    });
                                } else {
